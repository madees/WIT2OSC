...
class DeviceModel:
    ...
    def processData(self, Bytes):
        if Bytes[1] == 0x61:
            ...
            # D0-3 aux ports
            if Bytes[2] == 0x41:
                portD0 = self.getSignInt16(Bytes[5] << 8 | Bytes[4]) / 1024
                portD1 = self.getSignInt16(Bytes[7] << 8 | Bytes[6]) / 1024
                portD2 = self.getSignInt16(Bytes[9] << 8 | Bytes[8]) / 1024
                portD3 = self.getSignInt16(Bytes[11] << 8 | Bytes[10]) / 1024
                self.set("D0", round(portD0, 5))
